...
#include "RDC_CAN_platform_1071.h"
...
#define DRIVER_NAME         "RDC_CAN_platform_1071"   // RDC CAN Driver
...
static unsigned int io[MAX_DEVICES_ON_PLATFORM] = {
    [0 ... MAX_DEVICES_ON_PLATFORM-1] = 0
};

static unsigned int mem[MAX_DEVICES_ON_PLATFORM] = {
    [0 ... MAX_DEVICES_ON_PLATFORM-1] = 0
};

static unsigned int irq[MAX_DEVICES_ON_PLATFORM] = {
    [0 ... MAX_DEVICES_ON_PLATFORM-1] = 0
};
...
MODULE_PARM_DESC(mem, " Base mem address");
module_param_array(mem, uint, NULL, 0);
...
int __init Module_init(void)
{
    ...
    for (i = 0; i < MAX_DEVICES_ON_PLATFORM; i++)
    {
        ...
        if (mem[i] != 0 && irq[i] != 0)
        {
            ThisModule_supported_devices[i] = platform_device_alloc("fixed_17f3_1071_1", fixed_17f3_1071_1); // this_device_id->name and this_device_id->driver_data
            ...
            else
            {
                ...
                DrvDbgPrint(0, "    i = %d \n", i);
                DrvDbgPrint(0, "    mem:0x%x irq=%d \n", mem[i], irq[i]);
                
                memset(resources, 0, sizeof(resources));
                
                resources[0].start = mem[i];
                resources[0].end = mem[i] + 128 - 1;
